...
if up_f == "yes":
	summary.write("{0} circRNAs were given as input\n\tfor {1} circRNAs ({2:.1f} %), primer pairs were designed and filtered succesfully \n\tfor {3} circRNAs ({4:.1f} %), no primer pairs could be designed by primer3\n\tfor {12} circRNAs ({13:.1f} %), none of the primer pairs passed all filters\n\nin total\n\t{5} primer pairs were designed and tested\n\t{6} ({7:.1f} %) primer pairs passed all filters\n\t{8} ({9:.1f} %) primer pairs failed the specificity filter\n\t{10} ({11:.1f} %) primer pairs failed the secundary structure of the amplicon filter\n\nsee log file for more details per circRNA\n\n".format(total_nr_circ, primer_found, primer_found_perc, primer_no_design, primer_no_design_perc, total_nr_primers, primers_passed, primers_passed_perc, failed_spec, failed_spec_perc, failed_str_amp, failed_str_amp_perc, primer_not_found, primer_not_found_perc))
else:
